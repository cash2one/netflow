...
class NetFlowManager(object):
    def __init__(self, region):
        ...
        self.keyc = cclient = keystoneclient.v2_0.client.Client(username='admin',
                                                password='test',
                                                tenant_name='admin',
	...
	record_exist = None
        ...
        if not record_exist:
			...
			print 'AAAAAAA: %s  Network: %s' % (i,network_name)
                        max_in_rate += i["counter_volume"]
                    else:
			print '%s  Network_YYYY: %s' % (i,network_name)
                        continue
	    ...
	    '''
            ...
            print 'insert into netrate_project(in_rate,out_rate,date,region,project_id,' \
                  'begin_rate_date,end_rate_date) ' \
                  'values("%.2f","%.2f","%s","%s","%s","%s","%s")' % \
	...
	allLongID = self.get_instance_long_id_by_project_id(project_id)
        ...
        for i in allLongID:
            network_name, ipaddr = self.getNetworkNameByLongID(j)
            if network_name and self.networkIsMatch(network_name):
                ...
                try:
                    ...
                    LOG.error('json_str store error because of %s' % e)
...
def works(func, region, p_id):
    ...
    begin_time = datetime.datetime.strptime('2016-02-26 00:00:00', '%Y-%m-%d %H:%M:%S')
    today = datetime.datetime.strptime('2016-02-27 00:00:00', '%Y-%m-%d %H:%M:%S')
...
if __name__ == '__main__':
	...
	LOG.info('########################## Start ###########################')
        ...
        instance = NetFlowManager(region)
        p_id = ['620e1f372fa84bd09f42e9062ae8e621']
